...
- **Dreams**: `dreams-run` analiza conversaciones (24h/7d), Sonnet destila learnings y los escribe como `.md` en `<master>/dreams/`. **El agente ya NO lee `/dreams/` por filesystem** (con 231 archivos activos eso costaba listado + lecturas + turnos extra en CADA respuesta): tras cada corrida, `dreams-run` consolida TODOS los dreams activos en un **digest rodante** (`rebuildDigest`, ≤900 palabras, parte del digest anterior + activos) guardado en `runtime_config.DREAMS_DIGEST`, y `generate-response` lo inyecta al contexto de cada sesión como bloque `aprendizajes_del_operador` (**prioridad mayor que la voz base**, TTL 60s, sin re-sync). Las mutaciones del dashboard (aprobar/borrar/importar) disparan `dreams-run {digest_only:true}` fire-and-forget (`triggerDigestRebuild`, `lib/memory-list.ts`) para que el cambio llegue al agente en <60s. Cota de activos: `DREAMS_MAX_ACTIVE` (default 60) — el exceso más viejo se archiva a `/dreams-archive/` (fuera del dashboard y de futuras consolidaciones; su esencia persiste en el digest rodante). Cada learning lleva `severity` (sugerencia|advertencia|error, codificada también en el filename como `sug|adv|err`) y la política `runtime_config.DREAMS_AUTO_ACTIVATE` decide qué se activa solo: `all` (default del template), `error` (solo errores se auto-activan = autocorrección; el resto espera aprobación — **es la política configurada en KIA**) o `none`. Lo no activado va a `/dreams-pending/` (no entra al digest) y se aprueba/descarta desde `/dreams`. Todo dream `error` genera una alerta (`kind=dream_error`). Borrar un activo lo saca del digest en el próximo rebuild (<60s).
- **Paso de voz condicional**: si `<master>/voice/` está vacío, `composeSystem({hasVoice:false})` omite el paso de voz del CORE_SCAFFOLD (era un turno muerto por sesión; en KIA la voz vive en el SYSTEM_PROMPT). Detección en `detectVoiceFiles` (`sync-agent-tools.ts`), fail-open a `true`. El prompt vivo de KIA también se limpió de referencias a `/voice//dreams` (snapshot `agent/system-prompt.kia-live.md` en sync).
